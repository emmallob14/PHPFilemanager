		...
		IF(($SITEURL[1] == "doNotification") AND ISSET($_POST["remove_notice"])) {
			...
			if(isset($_POST["type"]) and isset($_POST["item_id"])) {
				...
				if($type == "login") {
					...
					$DB->execute("insert into _activity_logs set full_date=now(), date_recorded=now(), office_id='".$session->userdata("officeID")."', admin_id='$item_id', activity_page='login-notice', activity_id='$item_id', activity_details='$item_id', activity_description='A login attempt notification that was sent to you was removed'");
				}
				if($type == "pass_request") {
					#update the user login notification
					$DB->execute("delete from _admin_request_change where username='$item_id'");
					#add up the the users activity history
					$DB->execute("insert into _activity_logs set full_date=now(), date_recorded=now(), office_id='".$session->userdata("officeID")."', admin_id='$item_id', activity_page='password-change-notice', activity_id='$item_id', activity_details='$item_id', activity_description='A password change notification that was sent to you was removed.'");
				}
