...
<?php if(in_array(strtolower($SITEURL[0]), ARRAY("profile","adduser","offices"))) { ?>
...
<div id="user-nav" class="navbar navbar-inverse">
  <ul class="nav">
    <li  class="dropdown" id="profile-messages" ><a title="" href="#" data-toggle="dropdown" data-target="#profile-messages" class="dropdown-toggle"><i class="icon icon-user"></i>  <span class="text">Welcome <?php print $admin_user->get_details_by_id($session->userdata(":lifeID"))->funame; ?></span><b class="caret"></b></a>
      <ul class="dropdown-menu">
		...
		<li class="divider"></li>
		<li class=""><a title="" href="<?php print $config->base_url(); ?>Settings"><i class="icon icon-signout"></i> <span class="text">Server Settings</span></a></li>
		<?php } ?>
		<li class="divider"></li>
...
<!--start-top-serch-->
<div id="search">
  <input style="width:250px;" name="q" type="text" placeholder="Search here..."/>
...
<div id="sidebar"><a href="<?php print $config->base_url(); ?>Dashboard" class="visible-phone"><i class="icon icon-home"></i> Dashboard</a>
  <ul>
	...
	<?php } ?>
	<?php if($admin_user->confirm_super_user()) { ?>
	<li <?php if(in_array(($SITEURL[0]), array('Settings'))) { ?>class="active"<?php } ?>> <a href="<?php print $config->base_url(); ?>Settings"><i class="icon icon-signout"></i> <span>Server Settings</span> </a></li>
	<?php } ?>
	<li class="content"> <span>User Disk Space</span>
	  <?php
		// assign variables 
		// total disk usage
		$usage = ($directory->user_disk_info('SUM(item_size_kilobyte) AS item_size', "user_id='{$session->userdata(":lifeID")}'", 'item_size', 'ORDER BY id ASC')*1024);
		// total assigned usage
		$total = ($admin_user->get_details_by_id($session->userdata(":lifeID"))->upload_limit);
		// percent used 
	...
	<li class="content"> <span>Today Overall Uploads</span>
      ...
      <div class="bandwidth_stat"><?PHP PRINT $directory->return_usage()->today_used_size; ?> / <?php print  file_size_convert($offices->item_by_id('daily_upload', $session->userdata("officeID"))); ?></div>
    </li>
    <li class="content"> <span>Office Disk Space Usage</span>
      ...
      <div class="disk_stat"><?PHP PRINT $directory->return_usage()->file_size; ?> / <?php print  file_size_convert($offices->item_by_id('disk_space', $session->userdata("officeID"))); ?></div>
    </li>
	<?php if($admin_user->confirm_super_user()) { ?>
